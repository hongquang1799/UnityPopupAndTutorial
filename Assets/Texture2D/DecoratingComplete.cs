...
[ExecuteInEditMode]
public class DecoratingComplete : MonoBehaviour
{
    ...
    void Update()
    {
        for (int i = 0; i < cachedVertices.Length; i++)
        {
            ...
            float y = curve.Evaluate(Mathf.Clamp01(m + 0.5f));

            vertices[i].y = vertex.y * y;

            float v = vertex.x + width * (0.5f - x * 0.5f);
            vertices[i].x = v + (vertex.x - v) * curve2.Evaluate(Mathf.Clamp01(t - x + delay));
        }
